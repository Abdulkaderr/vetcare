...
class Bed extends MX_Controller {
    ...
    public function addBed() {
        ...
        $last_a_time = $this->input->post('last_a_time');
        $category = $this->input->post('category');
        $last_d_time = $this->input->post('last_d_time');

        $last_a_time1 = $last_a_time;
        $last_d_time1 = $last_d_time;

        $last_a_time1 = date("d/m/Y", strtotime($last_a_time1));
        $last_d_time1 = date("d/m/Y", strtotime($last_d_time1));
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                ...
                'bed_id' => $bed_id,
                'status' => $status,
                'last_a_time' => $last_a_time1,
                'last_d_time' => $last_d_time1
            );
            if (empty($id)) {
