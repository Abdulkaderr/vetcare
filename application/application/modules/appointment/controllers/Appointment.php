<?php
// orjinallllllllllllllllllllllllllllllll
...
class Appointment extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Nurse', 'Doctor', 'Patient', 'Receptionist', 'Owner'))) {
    ...
    public function addNew()
    {
        ...
        if ((empty($id))) {
            ...
            $owner_add_date = $add_date;
            $owner_registration_time = $registration_time;
        } else {
        ...
        $owner_id = rand(10000, 1000000);

        $this->load->library('form_validation');
        $this->form_validation->set_error_delimiters('<div class="error">', '</div>');

        if ($owner == 'add_new') {
            $this->form_validation->set_rules('p_name', 'Owner Name', 'trim|required|min_length[1]|max_length[100]|xss_clean');
            $this->form_validation->set_rules('p_phone', 'Owner Phone', 'trim|required|min_length[1]|max_length[100]|xss_clean');
        }

        // Validating Name Field
        $this->form_validation->set_rules('owner', 'Owner', 'trim|required|min_length[1]|max_length[100]|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($owner == 'add_new') {
                ...
                $data_p = array(
                    ...
                    'nid' => $p_nid,
                    // 'age' => $p_age,
                    // 'add_date' => $owner_add_date,
                    // 'registration_time' => $owner_registration_time,
                ...
                if ($this->ion_auth->email_check($p_email)) {
                    ...
                    $this->owner_model->insertOwner($data_p);
                    $owner_user_id = $this->db->get_where('owner', array('email' => $p_email))->row()->id;
            ...
            $owner_phone = $this->owner_model->getOwnerById($owner)->phone;
            if (empty($id)) {
                $room_id = 'hms-meeting-' . $owner_phone . '-' . rand(10000, 1000000);
            ...
            $ownername = $this->owner_model->getOwnerById($owner)->name;
            $doctorname = $this->doctor_model->getDoctorById($doctor)->name;
            $owner_details = $this->owner_model->getOwnerById($owner);
            ...
            $data_appointment = array(
                ...
                'user' => $user,
                'patient_name' => $owner_details->name,
                'patient_phone' => $owner_details->phone,
                'patient_address' => $owner_details->address,
            ...
            if (empty($id)) {     // Adding New department
                ...
                $owner_doctor = $this->owner_model->getOwnerById($owner)->doctor;
                ...
                $owner_doctors = explode(',', $owner_doctor);

                if (!in_array($doctor, $owner_doctors)) {
                    $owner_doctors[] = $doctor;
                    $doctorss = implode(',', $owner_doctors);
                    $data_d = array();
                    $data_d = array('doctor' => $doctorss);
                    $this->owner_model->updateOwner($owner, $data_d);
                }
                $response = $this->sendSmsDuringAppointment($id, $data, $owner, $doctor, $status);
                ...
                if (!empty($pay_now_appointment)) {
                    ...
                    $date = time();
                    $this->appointmentPayment($deposit_type, $data_for_payment, $owner, $doctor, $consultant_fee, $date, $inserted_id, $redirectlink);
                } else {
                ...
                if ($appointment_contingent->payment_status == 'unpaid') {
                    ...
                    if (!empty($pay_now_appointment)) {
                        ...
                        $date = time();
                        $this->appointmentPayment($deposit_type, $data_for_payment, $owner, $doctor, $consultant_fee, $date, $appointment_contingent->payment_id, $redirectlink);
                    }
                } else {
                    $this->appointment_model->updateAppointment($id, $data);
                }
    ...
    public function appointmentPayment($deposit_type, $data, $owner, $doctor, $consultant_fee, $date, $inserted_id, $redirectlink)
    {
        $owner_details = $this->owner_model->getOwnerById($owner);
        $user = $this->ion_auth->get_user_id();
        $doctorname = $this->doctor_model->getDoctorById($doctor)->name;
        if ($deposit_type == 'Card') {
            ...
            if ($gateway == 'PayPal') {
                ...
                $this->paystack->paystack_standard($amount_in_kobo, $ref, $owner, $inserted_id, $this->ion_auth->get_user_id(), $redirectlink);
                ...
                $this->sslcommerzpayment->request_api_hosted($data['grand_total'], $owner, $inserted_id, $this->ion_auth->get_user_id(), $redirectlink);
            } else {
    ...
    function sendSmsDuringAppointment($id, $data, $owner, $doctor, $status)
    {
        $set['settings'] = $this->settings_model->getSettings();
        $ownerdetails = $this->owner_model->getOwnerById($owner);
        ...
        $to = $ownerdetails->phone;
        $name1 = explode(' ', $ownerdetails->name);
        //  $length=count($name1);
        if (!isset($name1[1])) {
            $name1[1] = null;
        }
        ...
        if ($autoemail->status == 'Active') {
            ...
            if (empty($id)) {
                if ($status != 'Confirmed') {
                    ...
                    if ($ownerdetails->appointment_creation != 'Inactive') {
                        ...
                        $this->email->to($ownerdetails->email);
                    ...
                    if ($ownerdetails->appointment_confirmation != 'Inactive') {
                        $message1 = $autoemail->message;
                        $messageprint1 = $this->parser->parse_string($message1, $data1);
                        if ($mail_provider == 'Domain Email') {
                            $this->email->from($email_Settings->admin_email);
    ...
    function editAppointment()
    {
        ...
        $data['owners'] = $this->owner_model->getOwnerById($data['appointment']->owner);
    ...
    function editAppointmentByJason()
    {
        $id = $this->input->get('id');
        $data['appointment'] = $this->appointment_model->getAppointmentById($id);
        $data['owner'] = $this->owner_model->getOwnerById($data['appointment']->owner);
    ...
    function getAppointment()
    {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            $options3 = '<a class="btn green" title="' . lang('treatment') . '" style="color: #fff;" href="appointment/medicalHistory?id=' . $appointment->id . '"><i class="fa fa-stethoscope"></i> ' . lang('treatment') . '</a>';
    ...
    function getAppoinmentList()
    {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            // } else {
            //     $bill_status = '<span class=" label label-primary">' . lang('due') . '</span>';
            // }

            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist'))) {
                if ($payment_details->amount_received == NULL || $payment_details->amount_received == 0) {
                    $option2 = '<a class="btn delete_button" title="' . lang('delete') . '" href="appointment/delete?id=' . $appointment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"></i> ' . lang('') . '</a>';
                } else {
                    if ($this->ion_auth->in_group(array('admin'))) {
                        $option2 = '<a class="btn delete_button" title="' . lang('delete') . '" href="appointment/delete?id=' . $appointment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"></i> ' . lang('') . '</a>';
                    } else {
            ...
            $ownerdetails = $this->owner_model->getOwnerById($appointment->owner);
            if (!empty($ownerdetails)) {
                $ownername =/* ' <a type="button" class="history" data-toggle = "modal" data-id="' . $appointment->owner . '"> ' . */$ownerdetails->name /*. '</a>'*/;
            } //else {
            //     $ownername = ' <a type="button" class="history" data-toggle = "modal" data-id="' . $appointment->owner . '"> ' . $appointment->patientname . '</a>';
            // }
            ...
            //         if ($appointment->status == 'Confirmed') {
            //             $options7 = '<a class="btn btn-info btn-xs btn_width detailsbutton" title="' . lang('start_live') . '" style="color: #fff;" href="meeting/instantLive?id=' . $appointment->id . '" target="_blank" onclick="return confirm(\'Are you sure you want to start a live meeting with this owner? SMS and Email will be sent to the Owner.\');"><i class="fa fa-headphones"></i> ' . lang('live') . '</a>';
            //         } else {
            //             $options7 = '';
            //         }
            //     } else {
    ...
    function getConfirmedAppoinmentList()
    {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            $ownerdetails = $this->owner_model->getOwnerById($appointment->owner);
            if (!empty($ownerdetails)) {
                $ownername = //' <a type="button" class="history" data-toggle = "modal" data-id="' . 
               // $appointment->owner 
                //. '"> ' .
                //
                 $ownerdetails->name 
            ...
            // if ($this->ion_auth->in_group(array('Doctor'))) {
            //     if ($appointment->status == 'Confirmed') {
            //         $options7 = '<a class="btn btn btn-info btn-xs btn_width  detailsbutton" title="' . lang('start_live') . '" style="color: #fff;" href="meeting/instantLive?id=' . $appointment->owner . '" target="_blank" onclick="return confirm(\'Are you sure you want to start a live meeting with this owner? SMS and Email will be sent to the Owner.\');"><i class="fa fa-headphones"></i> ' . lang('live') . '</a>';
            //     } else {
            //         $options7 = '';
            //     }
            // } else {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            if ($owner == $appointment->owner) {
             ...
             //   $option2 = '<a class="btn btn-info btn-xs btn_width delete_button" href="appointment/delete?id=' . $appointment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"> </i></a>';
                $ownerdetails = $this->owner_model->getOwnerById($appointment->owner);
                if (!empty($ownerdetails)) {
                    $ownername = ' <a type="button" class="" data-toggle = "modal" data-id="' . $appointment->owner . '"> ' . $ownerdetails->name . '</a>';
                } else {
                    $ownername = ' <a type="button" class="" data-toggle = "modal" data-id="' . $appointment->owner . '"> ' . $appointment->patientname . '</a>';
                }
                $doctordetails = $this->doctor_model->getDoctorById($appointment->doctor);
                if (!empty($doctordetails)) {
                    $doctorname = $doctordetails->name;
