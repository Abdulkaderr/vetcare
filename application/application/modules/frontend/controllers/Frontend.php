...
class Frontend extends MX_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('frontend_2', $data);
    ...
    public function addNew()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array();
            $owner_info = $this->owner_model->getOwnerById($owner);
            ...
            $room_id = 'hms-meeting-' . $owner_info->phone . '-' . rand(10000, 1000000);
            ...
            if (empty($id)) {     // Adding New department
                ...
                $ff = $this->db->get_where('doctors_owners', array('doctor_id' => $doctor, 'owner_id' => $owner))->row()->id;
                if ($ff == null) {
                    $dataj = array(
                        'doctor_id' => $doctor,
                        'owner_id' => $owner,
                    ...
                    $this->owner_model->insertDoctorOwner($dataj);
                }
                //
                if (!empty($pay_now_appointment)) {
                    ...
                    $date = time();
                    $this->appointmentPayment($data_for_payment, $patient, $doctor, $consultant_fee, $date, $inserted_id, $redirectlink, $owner);
                } else {
                    ...
                    redirect('frontend/appointmentSuccess?id=' . $appointment_id);
                }
            }
    ...
    public function appointmentPayment($data, $patient, $doctor, $consultant_fee, $date, $inserted_id, $redirectlink, $owner)
    {
        ...
        if ($gateway == 'PayPal') {
            ...
            $ref = date('Y') . '-' . rand() . date('d') . '-' . date('m') . '-frontend';
    ...
    public function doctors()
    {
        ...
        $doctor_search = $this->input->post('doctor_search');

        //   var_dump($location_search);

        $data = array();
        $data['doctors'] = $this->doctor_model->getDoctor($location_search, $doctor_search);
        // var_dump($data['doctors']);

        $data['settings1'] = $this->settings_model->getSettings();
        $data['gateway'] = $this->pgateway_model->getPaymentGatewaySettingsByName($data['settings1']->payment_gateway);
        $this->load->view('frontend_header', $data);
        $this->load->view('frontend_doctors', $data);
        $this->load->view('frontend_footer', $data);
    }
    ...
    public function doctorDetails()
    {
        ...
        $this->load->view('frontend_header', $data);
        $this->load->view('frontend_doctor_details', $data);
        $this->load->view('frontend_footer', $data);
    }
    public function doctorBooking()
    {
        ...
        $this->load->view('frontend_header', $data);
        $this->load->view('frontend_doctor_booking', $data);
        $this->load->view('frontend_footer', $data);
    }
    ...
    public function missingPet()
    {
        ...
        $data = array();
        $data['missingpets'] = $this->missingpet_model->getMissingpet($location_search,$name_search);
       
        $data['settings1'] = $this->settings_model->getSettings();
        $this->load->view('frontend_header', $data);
        $this->load->view('frontend_missing_pet', $data);
        $this->load->view('frontend_footer', $data);
    }

    public function matingPet()
    {
        $location_search = '';
        $name_search = '';
        ...
        $data = array();
        $data['matingpets'] = $this->matingpet_model->getMatingpet($location_search,$name_search);
        // var_dump( count($data['matingpets']) );die;
        $data['settings1'] = $this->settings_model->getSettings();
        $this->load->view('frontend_header', $data);
        $this->load->view('frontend_mating_pet', $data);
        $this->load->view('frontend_footer', $data);
    }
    ...
    {
        $data = array();
        // $data['matingpets'] = $this->matingpet_model->getMatingpet();
        $data['settings1'] = $this->settings_model->getSettings();
        $this->load->view('frontend_header', $data);
        $this->load->view('frontend_owner_registration', $data);
        $this->load->view('frontend_footer', $data);
    }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($id)) {     // Adding New Owner
                if ($this->ion_auth->email_check($email)) {
                    ...
                    redirect('frontend/ownerRegistration');
                    // echo 'email p';
                } else {
                    $dfg = 11;
                    ...
                    $this->session->set_flashdata('feedback', lang('registration_successfull'));
                }
            }
    ...
    public function postaddfoundpet(){
        ...
        if ($this->upload->do_upload('pet_img')) {
            $path = $this->upload->data();
            $pet_img = "uploads/" . $path['file_name'];
            $data = array();
    ...
    public function addlostpets(){
        ...
        $data = array();
        $data['pets'] = $this->patient_model->getPatient();
        $data['lostpets'] = $this->frontend_model->getlostpets();
        $this->load->view('frontend_header', $data);
