...
class Patient extends MX_Controller {
    ...
    function __construct() {
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Nurse', 'Patient', 'Doctor', 'Laboratorist', 'Accountant', 'Receptionist', 'Owner'))) {
            redirect('home/permission');
        }
    }

    public function index($owner_id = 0) {
        if ($this->ion_auth->in_group(array('Patient'))) {
            redirect('home/permission');
        }

        $data['owner_idd'] = $owner_id;
    ...
    public function ownerpatient($owner_id = 0) {
        if ($this->ion_auth->in_group(array('Patient'))) {
            redirect('home/permission');
        }
        ...
        if ($owner_id == 0) {
            ...
            $data['owner_name'] = $this->db->get_where('owner', array('id' => $owner_id))->row()->name;
            $data['owner_nid'] = $this->db->get_where('owner', array('id' => $owner_id))->row()->nid;
            $data['owner_phone'] = $this->db->get_where('owner', array('id' => $owner_id))->row()->phone;
        }
    ...
    public function addNew() {
        ...
        $fruitleness = $this->input->post('fruitleness');
        // var_dump($fruitleness);die;
        ...
        //new 
        $p_strain = $this->input->post('p_strain');
...
// var_dump( $phone);die;
            ...
            if (empty($id)) {
                if ($this->ion_auth->email_check($email)) {
                    ...
                    //new patient number
                    $pnymber = 'CN2023' . sprintf("%'.05d\n", $patient_user_id);
                    $pnumber_info = array('patient_number' => $pnymber);
                    $this->patient_model->updatePatient($patient_user_id, $pnumber_info);
                    //
    ...
    function editPatientByJason() {
        ...
        $data['owner'] = $this->owner_model->getOwnerById($data['patient']->owner);
    ...
    function getPatient() {
        ...
        foreach ($data['patients'] as $patient) {
            ...
            $options3 = '<a class="btn green" title="' . lang('treatment') . '" style="color: #fff;" href="patient/medicalHistory?id=' . $patient->id . '"><i class="fa fa-stethoscope"></i> ' . lang('treatment') . '</a>';
            ...
            $options8 = ' <a href="patient/petdetails/'.$patient->id.'" class="btn detailsbutton inffo"><i class="fa fa-info"> </i> Details</a>';
$owner_info = $this->owner_model->getOwnerById($patient->owner);
            ...
            if ($this->ion_auth->in_group(array('Accountant', 'Receptionist'))) {
                $info[] = array(
                    ...
                    $patient->patient_number,
                    $this->settings_model->getSettings()->currency . $this->patient_model->getDueBalanceByPatientId($patient->id),
                    $options1 . ' ' . $options6 . ' ' . $options8 //. ' ' . $options4
                    ,
                );
            }
            ...
            if ($this->ion_auth->in_group(array('Laboratorist', 'Nurse', 'Doctor'))) {
                $info[] = array(
                    ...
                    $owner_info->nid,
                    $patient->patient_number,
                    $options1 . ' ' . $options6 . ' ' . $options3. ' ' . $options8,
                );
            }
    ...
    //new function get specific owner patients (pets)
    function getOwnerPatients($owner_idd) {
        $requestData = $_REQUEST;
        $start = $requestData['start'];
        $limit = $requestData['length'];
        $search = $this->input->post('search')['value'];
        ...
        if ($limit == -1) {
            if (!empty($search)) {
                $data['patients'] = $this->patient_model->getOwnerPatientBySearch($search, $order, $dir, $owner_idd);
                //$data['patients'] = $this->patient_model->getPatientBysearch($search, $order, $dir);
            } else {
                //$data['patients'] = $this->patient_model->getPatientWithoutSearch($order, $dir);
                $data['patients'] = $this->patient_model->getOwnerPatientWithoutSearch($order, $dir, $owner_idd);
            }
        } else {
            if (!empty($search)) {
                //$data['patients'] = $this->patient_model->getPatientByLimitBySearch($limit, $start, $search, $order, $dir);
                $data['patients'] = $this->patient_model->getOwnerPatientByLimitBySearch($limit, $start, $search, $order, $dir,$owner_idd);
            } else {
                //$data['patients'] = $this->patient_model->getPatientByLimit($limit, $start, $order, $dir);
                $data['patients'] = $this->patient_model->getOwnerPatientByLimit($limit, $start, $order, $dir, $owner_idd);
            }
        }
...
$owner_info = $this->owner_model->getOwnerById($patient->owner);
        ...
        if (!empty($info)) {
            $output = array(
                "draw" => intval($requestData['draw']),
                "recordsTotal" => $this->db->where('owner',$owner_idd)->get('patient')->num_rows(),
                "recordsFiltered" => $this->db->where('owner',$owner_idd)->get('patient')->num_rows(),
                "data" => $info
            );
        } else {
            $output = array(
    ...
    function getPatientPayments() {
        ...
        foreach ($data['patients'] as $patient) {
            ...
            $options4 = '<a class="btn btn-xs green" title="' . lang('payment') . ' ' . lang('treatment') . '" style="color: #fff;" href="finance/patientPaymentHistory?patient=' . $patient->id . '"><i class="fa fa-money-bill-alt"></i> ' . lang('payment') . ' ' . lang('treatment') . '</a>';
...
function odontogram() {
            ...
            public function getPatientByOwner() {
                $id = $this->input->get('id');
               // $description = $this->input->get('description');
                $patients = $this->patient_model->getPatientByOwnerId($id);
                $option = '<option value="">' . lang('select') . '</option>';
                foreach ($patients as $patient) {
                   
                        $option .= '<option value="' . $patient->id . '">' . $patient->name . '</option>';
            ...
            public function getPatientByOwnerForEdit() {
                $id = $this->input->get('id');
                ...
                foreach ($patients as $patient) {
                    if($patient->id == $patient_id){
                      $option .= '<option value="' . $patient->id . '" selected ="selected">' . $patient->name . '</option>';
                    }else{
                        $option .= '<option value="' . $patient->id . '">' . $patient->name . '</option>';
                    }
                }
                $data['response'] = $option;
                $data['patients'] = $option;
                echo json_encode($data);
            }
