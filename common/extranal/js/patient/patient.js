    ...
    $(".table").on("click", ".editbutton", function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#editPatientForm').find('[name="p_strain"]').val(response.patient.strain).end();
                $('#editPatientForm').find('[name="p_color"]').val(response.patient.color).end();
                ...
                $('#editPatientForm').find('[name="owner"]').val(response.patient.owner).end();
...
// console.log(response.owner);
                ...
                if (response.owner !== null) {
                    ...
                    if (response.owner.nid == null) {
                        response.owner.nid = '';
                    }
                    if (response.owner.phone == null) {
                        response.owner.phone = '';
                    }
                    var option2 = new Option(response.owner.name + '- NID: ' + response.owner.nid + '- Phone: ' + response.owner.phone, response.owner.id, true, true);
                } else {
                    var option2 = new Option(' ' + '-' + '', '', true, true);
                }
                $('#editPatientForm').find('[name="owner"]').append(option2).trigger('change');
    ...
    $(".table").on("click", ".inffo", function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('.fruitlenessClass').append(response.patient.fruitleness).end();
                $('.genderClass').append(response.patient.sex).end();
                $('.birthdateClass').append(response.patient.birthdate).end();
                $('.strainClass').append(response.patient.strain).end();
                $('.colorClass').append(response.patient.color).end();
...
$(document).ready(function () {
    ...
    var table = $('#editable-sample1').DataTable({
        ...
        "searchable": true,
        "ajax": {
            url: "patient/getOwnerPatients/" + $('#owner_idd').val(),
            type: 'POST',
        },
        scroller: {
            loadingIndicator: true
...
$(document).ready(function () {
    ...
    $("#owner_select").select2({
        placeholder: select_owner,
        allowClear: true,
        ajax: {
            url: 'owner/getOwnerinfoWithAddNewOption',
            type: "post",
            dataType: 'json',
            delay: 250,
            data: function (params) {
...
$(document).ready(function () {
    ...
    $('#mymodalbtn').click(function(){
        ...
        if ($("#owner_iddd").val() != 0) {
            ...
            var option2 = new Option($("#owner_name11").val() + '- NID: ' + $("#owner_nid11").val() + '- Phone: ' + $("#owner_phone11").val(), $("#owner_iddd").val(), true, true);
            $('#addnewpatient').find('[name="owner"]').append(option2).trigger('change');
...
$(document).ready(function () {
    $(".pos_client").hide();
    $(document.body).on("change", "#owner_select", function () {
        var v = $("select.owner_select option:selected").val();
        if (v === "add_new") {
            $(".pos_client").show();
        } else {
            $(".pos_client").hide();
