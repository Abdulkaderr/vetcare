    ...
    $(".appointment_edit").on("click", ".editAppointmentButton", function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                var option = new Option(response.owner.name + '-' + response.owner.id, response.owner.id, true, true);
                $('#editAppointmentForm').find('[name="owner"]').append(option).trigger('change');
