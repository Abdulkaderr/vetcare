  ...
  $(".table").on("click", ".editbutton", function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#editAppointmentForm")
          .find('[name="owner"]')
          .val(response.appointment.owner)
          .end();
        $("#editAppointmentForm")
          .find('[name="doctor"]')
        ...
        var option = new Option(
          response.owner.name + "-" + response.owner.id,
          response.owner.id,
          true,
          true
        );
...
$(document).ready(function () {
  ...
  var table = $("#editable-sample5").DataTable({
    ...
    buttons: [
      { extend: "copyHtml5", exportOptions: { columns: [0, 1, 2, 3, 4, 5, 5] } },
      { extend: "excelHtml5", exportOptions: { columns: [0, 1, 2, 3, 4, 5, 5] } },
      { extend: "csvHtml5", exportOptions: { columns: [0, 0, 1, 2, 3, 4, 5, 5] } },
      { extend: "pdfHtml5", exportOptions: { columns: [0, 1, 2, 3, 4, 5, 5] } },
      { extend: "print", exportOptions: { columns: [0, 1, 2, 3, 4, 5, 5] } },
    ],
    aLengthMenu: [
      [10, 25, 50, 100, -1],
      [10, 25, 50, 100, "All"],
